...
- 极端长宽比块：若超过 3000px 且按 30 万像素缩放后的预计宽度不足 512px，只在本地请求层沿横向空白带临时拆成约 1500px 的子块；
- 目录以及超长 H2/H3 的每个临时子块都会复制同一段标题头；聚合只保留第一份标题，正式预处理清单仍是一张完整请求图；
...
普通请求块实测耗时约 2.3～14.8 秒。极端长图临时切割属于本地模型容量适配，不改变官方 API 流程和原图语义坐标。
