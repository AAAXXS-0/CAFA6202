...
放回全局矩阵。模型省略的空位通过原图单元格墨迹分布做顺序对齐；结构差异只记录 warning，不会因为模型行列与预处理参考不同就直接退出。遇到无法解析或坐标冲突时保存 `merge_warning.json` 和原始响应，并继续同图后续切块及其他图片。

重试和 warning 都会打印“原图名 / 区域 / 切块名”。某个切块的临时故障退让耗尽后，只把该块记入原图的 `recognition_failures.json`，不写成功缓存；同图已经成功的切块缓存保留，其他切块和其他原图继续识别。一个区域有缺块时暂缓该区域合并，但不会阻塞下一个区域。一张图的所有区域/切块都尝试完仍有失败，才标记整图不完整并进入下一图。批次跑完后另存 `partial_results.csv`，存在失败时不生成正式提交；再次运行会命中所有成功切片，只补失败部分。

唯一不能自动补空的情况是像素重叠兜底块：它没有可靠的逻辑行列，不能猜一个全空矩阵而破坏全局对齐，只记录为失败并等待重跑或人工检查。预处理空表和逻辑网格块的模型空响应则都能安全补成与预处理行列完全相同的空 HTML。
