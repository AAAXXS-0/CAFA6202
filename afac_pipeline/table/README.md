...
flowchart TD
    ...
    G --> H["二维表格包络内检测覆盖率 ≥90% 黑线"]
    H --> V{"该方向黑线是否至少 5 条？"}
    V -- "是" --> I["该方向采用黑线"]
    V -- "否" --> W["该方向采用墨水 ≤1% 的长白带"]
    I --> X{"缩放仍清晰且逻辑格不超过 560？"}
    W --> X
    X -- "是" --> Y["从原图裁出整表请求图"]
    X -- "否" --> J["优先保留完整列，只沿逻辑行切块"]
    J --> K["后续块重复顶部表头"]
    J -. "极端宽图" .-> L["沿完整行列边界二维切块"]
    L --> M["横向后续块重复左侧行名列"]
    J -. "边界无法规划" .-> N["像素切片 + 自适应重叠兜底"]
    K --> RATIO["极扁切片只纵向拉伸至宽高比不超过 4:1"]
    M --> RATIO
    Y --> RATIO
    RATIO --> Z{"选择识别后端"}
...
5%。宽低密度带负责分开同图异表；每张分表再使用 1% 投影峰值和最低
0.05% 墨水比例保守取得分析框，并向外留约短边 1% 的余量。

内部边界统一使用灰度阈值 225。候选黑线在独立二维表格包络内必须有至少
90% 黑像素，一个方向至少找到 5 条才采用；否则该方向改用墨水比例不超过
...
负责的逻辑行列范围，`header/stub` 表示额外重复的表头行和行名列。

## 3. 切片原则

程序对横向和纵向分别执行“90% 黑线优先、白带兜底”。即使图片尺寸能
整体送入，只要逻辑总格子数超过 `max_logical_cells_per_tile=560`，仍会沿
完整逻辑行切块；空单元格也计入预算，因为 FireRed 同样要输出 `<td></td>`
标签。宽度能可靠缩放时始终保留完整列，极端宽图才同时沿行列二维切分。
后续块重复顶部 `repeat_header_rows` 行，默认 1 行。

网格确定后会删除只含表格线、完全没有文字的最外层逻辑空行，中间空行和
带字边缘行不动。宽高比超过 `max_vlm_aspect_ratio=4.0` 的请求图只做纵向
拉伸，防止视觉模型把多行误判成一条超长行；逻辑坐标和单元格顺序不变。
若边界仍不足，才使用带自适应重叠的像素切片。所有请求图最长边不超过
3900px。
...
失效。图表切片的纵向拉伸、逻辑格预算和外沿空行清理都发生在预处理阶段；
响应仍按检测到的逻辑坐标校验和合并。模型省略右侧纯空列时，合并器只补回
尾部空单元格；正文行缺失、额外列或坐标冲突仍会明确报错。
...
横线和竖线分别使用非等比例分析图的方案暂不实施，设计细节保存在[非等比例缩放方案备忘](./非等比例缩放方案备忘.md)。这里指检测阶段分别改变横纵比例，与已经实施的“极扁请求图只做纵向拉伸”不是同一件事。
