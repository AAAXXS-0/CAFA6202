...
def _axis_groups(
    ...
    while start < logical_count:
        ...
        while end <= logical_count and boundaries[end] - boundaries[start] <= available:
            end += 1
        end -= 1
...
def plan_grid_tiles(
    ...
    row_groups = _axis_groups(row_boundaries, max_side, repeat_header_rows)
    column_groups = _axis_groups(column_boundaries, max_side, repeat_stub_columns)
