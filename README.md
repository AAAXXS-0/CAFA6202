...
- [图表分支](afac_pipeline/table/README.md)：正式V25使用20%横向分表、分表后强晕染分析框、子表单张50%统一划线、黑线先擦除、行列独立黑白混合、二维自适应列晕染、滑动窗口表体和固定物理网格切块。列左右晕染采用人工确认的0%～1%。
- [图表粗切备用路线](afac_pipeline/table/归档/安全线粗切备用路线.md)：实验代码仍放在 `tests/图表粗切实验/`，不接入正式一键脚本。
- [最终CSV失败条件](afac_pipeline/最终CSV失败条件.md)：列出预处理、识别、缓存和100行合并阶段所有会阻止正式CSV生成的情况。
...
默认模式会复用或重试当前正式预处理；全部图片检查完成后，只要仍有 fatal，
就一次性打印图片名和原因，并在加载模型前停止。需要先让组员识别现有成功
图片时，可使用强制部分识别：

```bash
/usr/bin/python3 一键生成FireRed模型CSV.py --强制进入识别
```

强制模式会跳过没有成功预处理清单的图片，结果写入
`outputs/FireRed最终提交/强制识别部分结果/`，不会覆盖正式100行CSV。
...
5. 识别异常优先按图表物理 R×C 补空，整图异常再用空字符串占位；
6. 写出降级清单，并按模板生成严格 100 行的最终 CSV。
...
```bash
/home/zero/miniconda3/envs/AFAC2026/bin/python 一键生成最终CSV.py
```

也可以在依赖已经装进系统 Python 时执行：

```bash
./一键生成最终CSV.sh
```

默认模式把预处理当成进入 API 前的总闸门：

1. 长图和图表都会逐张检查；单张图片发生 fatal 时，先记录错误并继续下一张。
2. 两个分支全部检查完后，一次性打印所有失败图片、错误原因和报告文件位置。
3. 只要有一张预处理失败，就在创建 API 客户端之前停止，不调用任何 API。
4. 每完成一张就原子更新一次总清单；中途退出后再次运行，只重试没有
   完整单图清单的图片，成功图片显示“逐图缓存复用”。
5. 每个分支工作目录都会持续更新
   `预处理错误汇总.json`、`预处理进度与错误汇总.csv` 和
   `预处理进度与错误汇总.html`。
6. 每张图片目录都有 `中文中间产物/`，其中用中文硬链接整理原图、
   分表图、表体滑窗、黑白划线、最终边界、墨迹矩阵和实际识别切块；
   硬链接不会重复占用图片空间。

如果已经知道有少量图片预处理失败，但希望先识别其余已有成功清单，可显式执行：

```bash
/home/zero/miniconda3/envs/AFAC2026/bin/python 一键生成最终CSV.py --force-api
```

中文参数别名也可使用：

```bash
./一键生成最终CSV.sh --强制进入API
```
...
运行。三个一键入口默认开启强制完成：识别重试耗尽后，图表优先按失败切块的
预处理 R×C 补空；区域合并失败则保留整区 R×C 空表；无法局部恢复的整图用
空字符串占位。详细原因写入各工作目录的 `recognition_failures.json`，最终仍生成
严格 100 行 CSV。底层 `main.py run-tables/run-long` 默认保持严格模式，只有显式
传入 `--allow-degraded-output` 才启用相同行为。
...
4. **切块级最终失败**：某块所有退让都用尽后，记录原图名、区域和切块名；一键模式按该块预处理 R×C 补空并缓存为 `error-fallback`，严格模式仍判失败。
5. **原图级不完整**：一张图的所有块都尝试完仍有缺块，才标记该原图不完整，写入 `recognition_failures.json`，进入下一张原图；已经成功的块缓存保留，重跑只补缺失块。
6. **整批级结果**：一键模式保证每个模板文件名都有一行结果，并同时输出降级明细；严格模式只要仍有原图不完整，就只写 `partial_results.csv`。
