...
重试和 warning 都会打印“原图名 / 区域 / 切块名”。某个切块退让耗尽时，
只把该原图记入 `recognition_failures.json`，不写整图成功缓存；同图已经成功的
切块缓存保留，其他原图继续识别。批次跑完后另存 `partial_results.csv` 并统一
报告失败原图，不会拿缺图结果生成正式提交。再次运行时，完整原图缓存和成功
切块缓存都会命中，实际只补失败部分。
