...
class LongSemanticTest(unittest.TestCase):
    def test_model_ink_and_sequence_select_conservative_h2(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            image = Image.new("RGB", (600, 2048), "white")
            draw = ImageDraw.Draw(image)
            # 正文墨迹行高约 8px，H2 约 20px，H3 约 12px。
            ...
            self.assertEqual(sum(item.level == 2 for item in headings), 2)
            self.assertEqual(sum(item.level == 3 for item in headings), 2)
            self.assertEqual(debug["h2_count"], 2)
            h2_evidence = [item for item in evidence if item.used_as_h2_boundary]
            self.assertTrue(all(item.h2_score >= 0.62 for item in h2_evidence))
