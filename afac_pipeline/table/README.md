...
程序对横向和纵向分别执行“90% 黑线优先、白带兜底”。表格等比缩到
`max_vlm_side` 后如果仍能保留配置要求的分辨率，就整体送入模型；否则只
沿完整行列边界切块。纵向后续块会把顶部 `repeat_header_rows` 行拼回图片，
横向后续块会把左侧 `repeat_stub_columns` 列拼回图片，默认均为 1。
